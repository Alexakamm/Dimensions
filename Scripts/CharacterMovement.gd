...
func _get_input():
	...
	if is_on_floor(): # check if character body is on the floor (built-in function)
		if Input.is_action_pressed("ui_left"): # check if player submits input (e.g. key press) designated for moving left
			velocity += Vector2(-movement_speed,0)

		if Input.is_action_pressed("ui_right"): # check if player presses key for moving right
			velocity += Vector2(movement_speed,0)

		if Input.is_action_just_pressed("ui_up"): # Jump only happens when we're on the floor (unless we want a double jump, but we won't use that here)
			velocity += Vector2(1,-jump_height) # move character body up (negative y in Godot)
		
		if Input.is_action_pressed("ui_accept") and Global.at_portal:
			Global.at_portal = false
			Global.scene += 1
			if Global.scene == 1: 
				get_tree().change_scene_to_file("res://Scene/flatland.tscn")
			elif Global.scene ==2: 
				get_tree().change_scene_to_file("res://Scene/lineland.tscn")
			elif Global.scene == 3: 
				get_tree().change_scene_to_file("res://Scene/pointland.tscn")

	if not is_on_floor(): # check if character body is not on the floor (jumping or falling) 
		if Input.is_action_pressed("ui_left"): #move character left while they are airborn
		...
		if Input.is_action_pressed("ui_right"):#move character right while they are airborn
